...
app.post('/encryptfile', async(req, res) => {
    ...
    fileName = "./encrypt/" + file.name.split('.')[0] + new Date().getTime() + '.' + file.name.split('.')[1];
    console.log(req.files.file);
    await req.files.file.mv(fileName, function(err){
        if(err) {
    ...
    var process = script('python', ['./scripts/encrypt.py', fileName, file.name.split('.')[0]]);
...
app.post('/decryptfile', function(req, res){
    ...
    fileName = "./decrypt/" + file.name.split('.')[0] + new Date().getTime() + '.' + file.name.split('.')[1];
    ...
    var script = require("child_process").spawn;
    var process = script('python', ['./scripts/decrypt.py', fileName, file.name.split('.')[0]]);
    process.stdout.on('data', function(data) {
